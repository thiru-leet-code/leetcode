...
public class ValidateIPAddress {
    ...
    private boolean validIP4(String IP) {
        ...
        for ( String s :sa) {
            if ( (s.length() > 1 && s.startsWith("0")) || s.length() == 0) return false;
            try { if (Integer.valueOf(s )> 255 || Integer.valueOf(s ) <0) return false; }
    ...
    private boolean validIP6(String IP) {
        ...
        for ( String s :sa) {
            if (s.length() == 0 || s.length() > 4 || s.length() < 1  ) return false;
